...
collide with the host page's styles. The one deliberate exception is a
``body { padding-bottom }`` rule on narrow screens, which reserves room for the
bottom tab bar.
...
#gm-nav {
  ...
  font-size: .9rem; line-height: 1.2; position: relative; z-index: 30;
}
...
@media (max-width: 640px) {
  ...
  /* The bar leaves the flow on mobile — keep it from covering the page end. */
