...
| `activity_detail` | Full activity detail with lap splits, structured-workout interval/phase breakdown, HR zones, and weather conditions; multisport activities also break out each leg (swim / T1 / bike / T2 / run) under `sub_activities`; `gear` lists the shoes/bike used with their cumulative distance |
